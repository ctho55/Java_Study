...
// ** 실습
// 1. 통신 담당 객체 준비
var xhrObj; // XMLHttpRequest 객체를 저장할 변수를 전역변수로 선언 
function createXHR(){
	if(window.ActiveXObject){
		// 웹 브라우저가 IE5.0, IE6.0인 경우   (explor 버전이 낮은 경우)
		xhrObj = new ActiveXObject("Microsoft.XMLHTTP"); 
	}else if(window.XMLHttpRequest){
		//웹 브라우저가 IE7.0 이상, 크롬, 파이어폭스, 사파리, 오페라등의 경우
		xhrObj = new XMLHttpRequest("Microsoft.XMLHTTP"); 
	} 
} //createXHR 
...
function startMethod(){
	//2.1) 객체생성 
	createXHR();
	//2.2) Response(결과처리) 를 담당할 메서드를 지정 
...
// JS_Parameter , Get 방식  get방식 URL에 붙어서 parameter가면서 defalt로 자동 UTF-8이되어 전송 
...
// JS_Parameter , Post 방식 POST 는 body 에 데이터를 담아 보내며 content-Type 'UTF-8' 설정 해주어야한다. 
...
function resultF(){
	...
	// 여기서 4는 요청객체의 상태가 모든 데이터를 받을 수 있는 상태로 완료된 경우
		if(xhrObj.status==200){
		// 404 ,505 => 에러페이지 코드 200은 정상Response Status
		 	document.getElementById('resultArea').innerHTML
				='**서버 Response 입니다** => <br>'+xhrObj.responseText;
