...
public class Ex02_Calender {
	...
	public static void main(String[] args) {
		...
		System.out.printf("오늘은 %d 년 %d 월 %d 일 , 올해의 %d 번째 주 입니다.~ \n",yy,MM+1,dd,yy_w);

		int hh = now.get(Calendar.HOUR);
		int min = now.get(Calendar.MINUTE);
		int sec = now.get(Calendar.SECOND);
		int hd = now.get(Calendar.HOUR_OF_DAY); //  24
		int ap = now.get(Calendar.AM_PM); // AM_PM:0이면 Am ,아니면 PM
		
		String ampm = (ap==0)? "AM":"PM";
		
		System.out.printf("지금은 %s %d:%d:%d, 그리고 %d 시 입니다. \n",ampm,hh,min,sec,hd);

		// Formatter 활용 
		Formatter f = new Formatter();
		f.format("%tR%n", now);   // 시간을 R Type 으로 
		f.format("%tT%n", now);	  // 시간을 T Type 으로 
		f.format("%tr%n", now);	  // 시간을 r Type 으로
		f.format("%tD%n", now);	  // 시간을 D Type 으로
		f.format("%tF%n", now);	  // 시간을 F Type 으로
		
		
		f.format("%tc%n", now);	  // 시간을 c Type 으로
		// => 목 5월 27 14:44:32 KST 2021
